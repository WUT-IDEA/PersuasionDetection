...
def evaluate(pred, gold, CLASSES):
  ...
  r = recall_score(gold, pred, average=None)#, zero_division=1)
  p = precision_score(gold, pred, average=None)#, zero_division=1)
  f1 = 2*p*r/(p+r)
  return f1[1] #, micro_f1
...
if __name__ == '__main__':
  ...
  if validate_files(pred_file, gold_file, CLASSES):
    ...
    F1 = evaluate(pred_file, gold_file, CLASSES)
    logger.info("macro-F1={:.5f}".format(F1))
#    logger.info(F1, Acc)
    if args.log_to_file:
      print(F1)
...
#    f1 = evaluate(pred_file, gold_file, CLASSES)
#    print(f1)
#    logger.info("macro-F1={:.5f}\tmicro-F1={:.5f}".format(f1))
#    if args.log_to_file:
#      print("{}\t{}".format(f1))
